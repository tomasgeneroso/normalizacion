let logger=require('./util/objetPrinter.js')
...
const productos=[
  ...
  const productSchema=new schema.Entity('producto')
  const productosfinal= new schema.Entity('productosfinal',{
      producto:productSchema
  })
  //NORMALIZACION
  let normalizado=normalize(productos,productosfinal)
 //logger(normalizado)
  
  console.log("normal-->",JSON.stringify(productos).length)
  console.log("normalizada-->",JSON.stringify(normalizado).length)
  
  //DESNORMALIZACION
  let desnormalizada=denormalize(normalizado.result,productosfinal,normalizado.entities)
  
  console.log('desnormalizada',JSON.stringify(desnormalizada).length)
  logger(desnormalizada)   
